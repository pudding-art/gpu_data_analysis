...
def process_trace_events(trace_events):
    return [(event["ts"] * 1e3, event["ts"] * 1e3 + event["dur"] * 1e3) for event in trace_events if "ts" in event and "dur" in event]
...
def calculate_empty_time_up_to_tag(empty_intervals, tag):
    ...
    if tag < prev_end:
        return prev_empty_time + (tag - prev_start)
    else:
        if index < len(empty_intervals):
            return empty_intervals[index][2]
        else:
            return prev_empty_time + (prev_end - prev_start)
