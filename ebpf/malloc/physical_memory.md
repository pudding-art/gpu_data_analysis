...
- U（User）：表示该页面可以被用户模式下的进程访问。处理器处于较低特权级别（如**x86架构中的ring 3**），只能执行有限的指令集，访问受限的资源。用户程序（如应用程序）通常在这个模式下运行。
- K（Kernel）：表示该页面只能被内核模式下的进程访问。处理器处于最高特权级别（如**x86架构中的ring 0**），具有完全访问计算机所有资源和指令的权限。操作系统内核和设备驱动程序通常在这个模式下运行。

特权模式权限和非特权模式权限大致对应于kernel模式权限和user模式权限，但特权模式权限是更广泛的概念，涵盖了处理器的执行模式和权限级别，而kernel和user模式权限主要侧重于操作系统中对资源和执行环境的划分。
...
这三个文件 `/proc/<pid>/oom_adj`、`/proc/<pid>/oom_score` 和 `/proc/<pid>/oom_score_adj` 的区别如下：
-  `/proc/<pid>/oom_adj`用于调整进程的OOM score，取值范围为-16到15，取值越小，进程越不容易被OOM killer终止。该接口较旧，主要在Linux 2.6内核中引入；
- `/proc/<pid>/oom_score`显示当前进程的OOM score，该分数根据进程的内存使用情况、运行时间等因素动态计算得出。该文件是一个只读文件，用户不能直接修改其值，用于了解进程当前的OOM score。
- `/proc/<pid>/oom_score_adj`用于调整进程的OOM score，取值范围为-1000到1000，取值越小，进程越不容易被OOM killer终止。该接口较新，从Linux 3.5开始引入，功能与`oom_adj`类似，但提供了更精细的调整范围。

在实际使用中，`/proc/<pid>/oom_score_adj` 更常被使用，因为它提供了更广泛的调整范围和更灵活的控制方式，可以用来保护关键进程，并管理非关键进程的内存使用，`oom_adj` 提供了一种灵活的方式来微调系统的内存管理策略，允许管理员根据实际需求和应用的重要性来调整进程的内存使用优先级。通过oom_adj调整进程的oomscore:
```shell
cd /proc/${pid}
echo ${value} > oom_adj
```
![oom_adj](image-35.png)
通过oom_score_adj调整进程的oom score:
```shell
cd /proc/${pid}
echo ${value} > oom_score_adj
```

⚠️注意：
- 修改 `oom_adj` 值可能会影响系统的稳定性，特别是在设置过高或过低的值时。
- 在生产环境中，应该谨慎使用 `oom_adj`，并根据实际需求和测试结果进行调整。
- 除了 `oom_adj`，还可以通过其他机制（如cgroups）来更精细地控制进程的内存使用。
...
⚠️Android在程序退出的时候，并不杀死进程，而是等OOM再杀死进程；Android根据不同的进程类型设置不同的oom_obj。
